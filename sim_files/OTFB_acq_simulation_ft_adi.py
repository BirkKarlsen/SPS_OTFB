...
parser.add_argument("--study_osc", "-so", type=int,
                    help="Option to plot and save the possible oscillation of the 3-sec power, defalt is False (0)")
...
parser.add_argument("--intensity_ramp", "-ir", type=float,
                    help="Option to set how long the intensity ramp is, default is 0.5 (50% of total sim-time)")
parser.add_argument("--track_totind", "-tt", type=int,
                    help="Option to track impedance (1) or not track impedance (0) turn by turn, default is 1")
...
if args.intensity_ramp is not None:
    int_ramp = args.intensity_ramp
if args.track_totind is not None:
    TRACK_IMP = bool(args.track_totind)


# Intensity ramp
n_ramp = int(N_t * int_ramp)
target_intensity = 3385.8196 * 10**10
start_intensity = 1e10

intensity_ramp = np.linspace(start_intensity, target_intensity, n_ramp)
...
beam = Beam(SPS_ring, int(np.sum(n_macro[:N_bunches])), intensity_ramp[0])
...
OTFB = SPSCavityFeedback(rfstation, beam, profile, post_LS2=True, V_part=V_part,
                         ...
                         G_llrf=llrf_g)   # TODO: change back to only 20
...
if SAVE_RESULTS:
    ...
    np.save(lxdir + f'without_impedance/profile_data/generated_profile_{fit_type}_{N_bunches}', profile.n_macroparticles)
    np.save(lxdir + f'without_impedance/profile_data/generated_profile_bins_{fit_type}_{N_bunches}', profile.bin_centers)
...
if not GENERATE:
    ...
    fwhm_arr = np.zeros((288, N_t//dt_ptrack))
    pos_arr = np.zeros((288, N_t//dt_ptrack))
    pos_fit_arr = np.zeros((288, N_t//dt_ptrack))
    ...
    for i in range(N_t):
        ...
        if i < n_ramp:
            beam.intensity = intensity_ramp[i]
