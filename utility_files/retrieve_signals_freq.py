'''
File to retrieve files from frequency scans and put them into a folder.
...
for i in range(len(freq_array)):
    sim_folder_i = f'scan_fr{freq_array[i]*1e-3:.0f}kHz_vc{VOLT_CONFIG}_ve{100 * v_error:.0f}' \
    ...
    os.system(f"cp {sim_dir_i}{file_i} {save_dir + save_name}{file_i[:-4]}"
              f"_fr{freq_array[i]*1e-3:.0f}kHz_vc{VOLT_CONFIG}_ve{100 * v_error:.0f}"
